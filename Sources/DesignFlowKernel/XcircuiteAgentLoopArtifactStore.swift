...
extension XcircuiteWorkspaceStore {
    ...
    public func writeLoopIterationSummaries(
        ...
        let relativePath = "\(XcircuiteWorkspace.directoryName)/runs/\(runID)/loop/iterations.jsonl"
        ...
        let lines = try summaries.map { summary in
            ...
            guard let text = String(data: data, encoding: .utf8) else {
                throw XcircuiteWorkspaceError.encodeFailed("loop iteration summary was not UTF-8")
            }
            return text
        }
    ...
    public func writeAgentLoopSnapshot(
        ...
        let relativePath = "\(XcircuiteWorkspace.directoryName)/runs/\(snapshot.runID)/loop/snapshot.json"
        let url = projectRoot.appending(path: relativePath)
        try ensureDirectory(at: url.deletingLastPathComponent())
    ...
    public func writeRunGuardVerdict(
        ...
        let relativePath = "\(XcircuiteWorkspace.directoryName)/runs/\(verdict.runID)/loop/guard-verdict.json"
        let url = projectRoot.appending(path: relativePath)
        try ensureDirectory(at: url.deletingLastPathComponent())
