...
public struct XcircuiteProjectManifest: Sendable, Hashable, Codable {
    ...
    public internal(set) var identity: XcircuiteProjectIdentity
    public internal(set) var files: [XcircuiteFileReference]
    public internal(set) var runs: [XcircuiteRunReference]

    init(
        identity: XcircuiteProjectIdentity,
        files: [XcircuiteFileReference] = [],
        runs: [XcircuiteRunReference] = []
    ) {
    ...
    public init(from decoder: Decoder) throws {
        ...
        identity = try container.decode(XcircuiteProjectIdentity.self, forKey: .identity)
        files = try container.decode([XcircuiteFileReference].self, forKey: .files)
        runs = try container.decode([XcircuiteRunReference].self, forKey: .runs)
    ...
    func validate() throws {
        ...
        for reference in files {
            ...
            if let producedByRunID = reference.producedByRunID {
                try XcircuiteIdentifierValidator().validate(producedByRunID, kind: .runID)
            }
            if let verifiedByRunID = reference.verifiedByRunID {
    ...
    private func validateRunManifestProjection(
        ...
        guard let pathRunID,
              reference.producedByRunID == pathRunID,
